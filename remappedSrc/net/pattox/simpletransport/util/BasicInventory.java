...
public interface BasicInventory extends SidedInventory {
    ...
    /**
     * Checks if an item of an ItemStack is in the inventory
     * @param stack The Stack to check
     * @return True when found, false when not found
     */
    default boolean containsStack(ItemStack stack) {
        for (int i = 0; i < size(); i++) {
            ItemStack filterStack = getStack(i);
            if (Objects.equals(filterStack.getItem(), stack.getItem())) {
                return true;
            }
        }
        return false;
